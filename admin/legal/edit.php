...
if (isset($_POST['update'])) {
    ...
    $data = mysqli_query($koneksi, "SELECT gambar FROM legal WHERE id='$id'");
    ...
    if (empty($kode) || empty($nama) || empty($keterangan) || empty($jasa) || empty($produk) || empty($harga)) {
        ...
        if (!empty($filename)) {
            ...
            $result = mysqli_query($koneksi, "UPDATE legal SET kode='$kode', nama='$nama',keterangan='$keterangan',jasa='$jasa',produk='$produk',harga='$harga', gambar='$filename' WHERE id=$id");
        }

        // MEMASUKAN DATA YANG DI UPDATE KECUALI GAMBAR
        $result = mysqli_query($koneksi, "UPDATE legal SET kode='$kode', nama='$nama',keterangan='$keterangan',jasa='$jasa',produk='$produk',harga='$harga' WHERE id=$id");
...
$result = mysqli_query($koneksi, "SELECT * FROM legal WHERE id=$id");
